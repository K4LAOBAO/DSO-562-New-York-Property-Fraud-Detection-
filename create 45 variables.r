...
data=data %>%
  mutate(BLDVOL= data$BLDAREA*data$STORIES)
...
head(data$FULLVAL_BLDAREA_TAXCLASS)
#FULLVAL_BLDVOL
...
data=data %>%
  order_by(data$RECORD)
  
dim(new_data)
new_data=data[,c(6,48:92)]


head(new_data$FULLVAL_BLDVOL_TAXCLASS)

head(data$BLDVOL)

write.csv(new_data, file = "Final_Data_updated.csv",row.names=FALSE)
